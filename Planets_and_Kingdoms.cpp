...
void dfs2(int node,vector<vector<int>>& graph,vector<int>& comps,int scc_count){
    visited[node]=1;
    ssc_id[node]=scc_count;
    for(auto child : graph[node]){
        if(!visited[child]){
            comps.push_back(child);
            dfs2(child,graph,comps,scc_count);
        }
    }
}
...
void solve(){
    ...
    cout<<scc_count<<'\n';
    for(int i=1;i<=n;i++)cout<<ssc_id[i]+1<<' ';
    cout<<"\n";
}
