...
void solve(){
    ...
    dfs(1);
    // vector<ll> euler(in.size());
    // for(ll i=0;i<in.size();i++){
    //     euler[i]=values[in[i]];
    // }
    SegmentTree<ll> tree(in,0,[&](ll a,ll b){
        return a+b;
    });
    while(q--){
        ll type;
        ...
        if(type==1){
            ll s,x;
            cin>>s>>x;
            ll strt_index=start[s];
            ll end_index=finish[s];
            tree.update(strt_index,x);
            ...
            ll strt_index=start[s];
            ll end_index=finish[s];
            ll ans=tree.query(strt_index,end_index);
            cout<<ans/2<<"\n";
        }
    }
}
